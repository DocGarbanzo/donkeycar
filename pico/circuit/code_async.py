...
async def read(serial, input_pins, output_pins, led, is_setup, count):
...
async def write(serial, input_pins, write_dict):
...
async def program():
    ...
    try:
        while True:
            # reading input
            is_setup = await read(serial, input_pins, output_pins, led,
                                  is_setup, count)
            # sending output, catching number of bytes written
            if is_setup:
                n = await write(serial, input_pins, write_dict)
            toc = time.monotonic()
            total_time += toc - tic
            tic = toc
            count += 1
...
async def main():
    prgm = asyncio.create_task(program())
    await asyncio.gather(prgm)
