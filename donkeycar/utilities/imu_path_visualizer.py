...
This module provides utilities for visualizing and analyzing IMU path data,
including real-time plotting and post-processing visualization with drift correction.
"""
...
def correct_loop_drift(df, min_loop_distance=1.0, max_distance=0.5, max_loops=10):
    """
    Correct drift in closed-loop IMU path data using iterative loop-by-loop approach.
    Uses weighted average reversal point detection for accurate loop end identification.
    Detects and corrects multiple loops until no more are found or max_loops reached.
    ...
    Args:
        ...
        max_distance (float): Maximum distance from origin to start looking for reversal point
    ...
    def show_sample_points(data, start_idx, end_idx, label, show_timestamps=True):
        """Show 10 equidistant sample points from a range"""
        if end_idx <= start_idx:
            print(f"  {label}: No points to show")
            return
            
        indices = np.linspace(start_idx, end_idx, min(10, end_idx - start_idx + 1), dtype=int)
        ...
        for i, idx in enumerate(indices):
            ...
            if show_timestamps:
                t = data.iloc[idx]['t']
                print(f"    [{i+1}] idx={idx}, t={t:.1f}, pos=[{x:.3f}, {y:.3f}]")
            else:
                print(f"    [{i+1}] idx={idx}, pos=[{x:.3f}, {y:.3f}]")

    def find_loop_end(data, start_idx, min_distance_traveled, max_distance_from_origin):
        """
        ...
        Args:
            ...
            min_distance_traveled (float): Minimum distance to travel before considering loop end
            max_distance_from_origin (float): Max distance from origin to start looking for reversal
            
        Returns:
            int or None: Index of loop end point, or None if not found
        """
        if start_idx >= len(data) - 3:  # Need at least 3 points for weighted average
            return None
        ...
        # Get loop start position (origin for first loop, actual position for subsequent loops)
        if start_idx == 0:
            loop_start_pos = np.array([0.0, 0.0])  # Origin for first loop
        else:
            loop_start_pos = np.array([data.iloc[start_idx]['x'], data.iloc[start_idx]['y']])
        
        print(f"  Looking for loop end starting at index {start_idx}")
        print(f"  Loop start position: [{loop_start_pos[0]:.3f}, {loop_start_pos[1]:.3f}]")
        print(f"  Parameters: min_distance={min_distance_traveled:.1f}m, max_distance={max_distance_from_origin:.1f}m")
        ...
        for i in range(len(cumulative_distance)):
            if cumulative_distance[i] >= min_distance_traveled:
                ...
                print(f"  Minimum distance {min_distance_traveled:.1f}m reached at index {min_distance_idx}")
                break
        ...
        # Phase 2: Find when we get close to the loop start (within max_distance)
        vicinity_start_idx = None
        print(f"  Searching for vicinity from index {min_distance_idx} to {len(data)-1}")
        
        # Debug: show some sample distances
        sample_indices = list(range(min_distance_idx - start_idx, min(min_distance_idx - start_idx + 50, len(cumulative_distance)), 10))
        print(f"  Sample distances after min_distance threshold:")
        for i in sample_indices:
            actual_idx = start_idx + i
            if actual_idx >= len(data):
                break
            current_pos = np.array([data.iloc[actual_idx]['x'], data.iloc[actual_idx]['y']])
            distance_to_start = np.linalg.norm(current_pos - loop_start_pos)
            print(f"    Index {actual_idx}: pos=[{current_pos[0]:.3f}, {current_pos[1]:.3f}], distance={distance_to_start:.3f}m")
        ...
        for i in range(min_distance_idx - start_idx, len(cumulative_distance)):
            ...
            if distance_to_start <= max_distance_from_origin:
                ...
                print(f"  Entered vicinity of loop start at index {vicinity_start_idx}, distance={distance_to_start:.3f}m")
                break
        
        if vicinity_start_idx is None:
            print(f"  Never got within {max_distance_from_origin:.1f}m of loop start")
            print(f"  Try increasing max_distance parameter (current: {max_distance_from_origin:.1f}m)")
            return None
        
        # Phase 3: Find reversal point using weighted average of 3 points
        print(f"  Searching for reversal point starting from index {vicinity_start_idx}")
        
        # Calculate distances to loop start for all remaining points
        distances_to_start = []
        for i in range(vicinity_start_idx, len(data)):
            current_pos = np.array([data.iloc[i]['x'], data.iloc[i]['y']])
            dist = np.linalg.norm(current_pos - loop_start_pos)
            distances_to_start.append(dist)
        
        # Find reversal point: look for where distance stops decreasing and starts increasing
        # Use weighted average to smooth out measurement noise
        
        if len(distances_to_start) < 7:  # Need at least 7 points for proper reversal detection
            print(f"  Not enough points for reversal detection")
            return None
            
        # Find the LAST reversal point (not the first) - the actual loop closure
        # Look for all potential reversal points and select the one with minimum distance
        potential_reversals = []
        ...
        if potential_reversals:
            # Filter for reversals that are reasonably close (within 2x of max_distance)  
            good_reversals = [r for r in potential_reversals if r[1] <= max_distance_from_origin * 2]
            
            if good_reversals:
                # Prioritize reversals that occur soon after entering vicinity
                # Use a weighted score: earlier time + closer distance
                vicinity_window = min(2000, len(data) - vicinity_start_idx)  # Look within next 2000 points
                
                scored_reversals = []
                for reversal in good_reversals:
                    idx, distance, next_avg = reversal
                    # Only consider reversals within reasonable window of vicinity start
                    if idx <= vicinity_start_idx + vicinity_window:
                        time_factor = (idx - vicinity_start_idx) / vicinity_window  # 0 = earliest, 1 = latest
                ...
                if scored_reversals:
                    ...
                    print(f"  Found {len(potential_reversals)} potential reversals, {len(good_reversals)} within 2x max_distance")
                    print(f"  {len(scored_reversals)} within vicinity window, selected best scoring at index {reversal_idx}")
                    print(f"    Score: {best_score:.3f} (lower=better), Distance: {current_avg:.3f}m")
                    print(f"    Distance increase: {((next_avg/current_avg - 1)*100):.1f}%")
                    return reversal_idx
                else:
                    # No reversals in vicinity window, take earliest good one
                    best_reversal = min(good_reversals, key=lambda x: x[0])
                    reversal_idx, current_avg, next_avg = best_reversal
                    print(f"  No reversals in vicinity window, selected earliest good reversal at index {reversal_idx}")
                    return reversal_idx
            else:
                # Fallback: if no good reversals, take the earliest of all
                best_reversal = min(potential_reversals, key=lambda x: x[0])
                reversal_idx, current_avg, next_avg = best_reversal
                print(f"  Found {len(potential_reversals)} potential reversals, none within 2x max_distance")
        ...
        print(f"  No clear reversal found, using minimum distance point at index {min_distance_idx}")
    ...
    print(f"Parameters: min_loop_distance={min_loop_distance}, max_distance={max_distance}, max_loops={max_loops}")
    ...
    while loop_num <= max_loops:
        ...
        loop_end_idx = find_loop_end(corrected_df, current_loop_start, min_loop_distance, max_distance)
        ...
        print(f"{loop_num}) LOOP {loop_num} END FOUND: index={loop_end_idx}, timestamp={t:.1f}")
        
        loop_end_indices.append(loop_end_idx)
        
        # Calculate loop drift
        loop_start_pos = np.array([corrected_df.iloc[current_loop_start]['x'], corrected_df.iloc[current_loop_start]['y']])
        loop_end_pos = np.array([corrected_df.iloc[loop_end_idx]['x'], corrected_df.iloc[loop_end_idx]['y']])
        loop_drift = loop_end_pos - loop_start_pos
        
        print(f"{loop_num+1}) CORRECTION AMOUNT for Loop {loop_num}: [{loop_drift[0]:.3f}, {loop_drift[1]:.3f}]")
        
        # Show loop points before correction
        print(f"{loop_num+2}) LOOP {loop_num} BEFORE CORRECTION:")
        show_sample_points(corrected_df, current_loop_start, loop_end_idx, f"Loop {loop_num} points before correction")
        ...
        if loop_length > 0:
            for i in range(current_loop_start, loop_end_idx + 1):
                ...
                corrected_df.iloc[i, corrected_df.columns.get_loc('x')] -= correction[0]
                corrected_df.iloc[i, corrected_df.columns.get_loc('y')] -= correction[1]
        
        # Debug: Check the actual corrected end position
        corrected_end_pos = np.array([corrected_df.iloc[loop_end_idx]['x'], corrected_df.iloc[loop_end_idx]['y']])
        print(f"   DEBUG: Loop {loop_num} end position after correction: [{corrected_end_pos[0]:.6f}, {corrected_end_pos[1]:.6f}]")
        
        # Show loop points after correction
        print(f"{loop_num+2}) LOOP {loop_num} AFTER CORRECTION:")
        show_sample_points(corrected_df, current_loop_start, loop_end_idx, f"Loop {loop_num} points after correction")
        ...
        if loop_end_idx < len(corrected_df) - 1:
            ...
            show_sample_points(corrected_df, loop_end_idx + 1, len(corrected_df) - 1, "Remaining points before shift")
            
            # Apply full drift correction to all remaining points
            for i in range(loop_end_idx + 1, len(corrected_df)):
                corrected_df.iloc[i, corrected_df.columns.get_loc('x')] -= loop_drift[0]
                corrected_df.iloc[i, corrected_df.columns.get_loc('y')] -= loop_drift[1]
            
            print(f"{loop_num+3}) REMAINING POINTS AFTER GLOBAL SHIFT:")
            show_sample_points(corrected_df, loop_end_idx + 1, len(corrected_df) - 1, "Remaining points after shift")
    ...
    for i, end_idx in enumerate(loop_end_indices):
        ...
        print(f"    End position: [{corrected_df.iloc[end_idx]['x']:.6f}, {corrected_df.iloc[end_idx]['y']:.6f}]")
    
    print(f"\nFinal result: {len(corrected_df)} points with {num_loops_found} loops corrected")
    print(f"Final data range: x=[{corrected_df['x'].min():.3f}, {corrected_df['x'].max():.3f}], y=[{corrected_df['y'].min():.3f}, {corrected_df['y'].max():.3f}]")
    
    return corrected_df, loop_end_indices


def visualize_imu_path(csv_file='imu.csv', correct_drift=True, min_loop_distance=1.0, max_distance=0.5):
    """
    ...
    if correct_drift:
        ...
        df, loop_end_indices = correct_loop_drift(df, min_loop_distance, max_distance)
        ...
        print(f"  Data range for plot: x=[{df['x'].min():.3f}, {df['x'].max():.3f}], y=[{df['y'].min():.3f}, {df['y'].max():.3f}]")
    ...
    def update_plot(val):
        ...
        if len(current_data) > 0:
            ...
            # Update text displays with separate time and position
            current_datetime = datetime.fromtimestamp(current_time_val)
            current_idx = len(current_data) - 1
            
            # Calculate distance to origin
            distance_to_origin = np.sqrt(last_point["x"]**2 + last_point["y"]**2)
            
            speed_text.set_text(f'Speed: {last_point["v"]:.2f}')
            time_text.set_text(f'Time: {current_datetime.isoformat(timespec="milliseconds")}')
            pos_text.set_text(f'Position: [{last_point["x"]:.3f}, {last_point["y"]:.3f}]')
            
            # Update debug info
            debug_text.set_text(f'Index: {current_idx} | Dist to origin: {distance_to_origin:.3f}m')
    ...
    # Add legend in top area underneath speed/time/position/loop displays - vertical arrangement
    ax.legend(bbox_to_anchor=(0.02, 0.77), loc='upper left', 
